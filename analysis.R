...
df_zip_gun <- df_gun |> 
  group_by(incident_zip) |> # Grouping by zip code. 
  summarise(instances = round(n())) |> # Summing the number of rows. 
  filter(!(incident_zip %in% c("00000", "99999", ""))) |> # Taking out invalid zip codes. 
  rename(zip = incident_zip)

df_year_gun <-  df_gun |> 
  group_by(year) |> 
  filter(year < 2025 & year > 2011) |>  # Filtering between 2012 to 2024. 
...
zip_gun_rate <- df_zip_gun |> 
  ...
  mutate(instance_per_100k = round((instances/juve_population)*100000,2)) |> # Making instances per 100k.  
...
zip_grad_rate_avg <- zip_grad_rate |> 
  mutate(
    average_grad_rate = round( 
      rowMeans( # Summing values across rows since we just want one average graduation. data for each zip code. 
...
regression_table_1 <- lm(instance_per_100k ~ average_grad_rate, data = regression) # Regression 1

regression_table_2 <- lm(average_grad_rate ~ instance_per_100k, data = regression) # Regression 2
